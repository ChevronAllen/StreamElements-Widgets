...
let windowHeight= $('main').height();
  ...
  function onMessage(event) {
    ...
    const xRange = Math.max(windowWidth - width, 0);
    const yRange = Math.max(windowHeight - height, 0);
    
    $(currentMessage).css({
      "left": `${ (FieldData.padding + Math.random() * xRange) }px`,
      "top":  `${ (FieldData.padding + Math.random() * yRange) }px`
    })//.fadeOut(5000 , () => deleteMessage(msgId))
  ...
  function MessageComponent(props) {
    ...
    let containerClasses = ['bubble','zoomIn','zoomOut']
    switch (messageType) {
      case 'highlight': {
    ...
    return Component('section', {
      ...
      children: [
        ...
        Component('div', { class: 'message', children: [ Component('span', { class: 'message-wrapper', children: [...parsedElements] })] }),
        RollComponent()
      ],
    })
  }
